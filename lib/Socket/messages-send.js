ñimport NodeCache from '@cacheable/node-cache';
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            if (Array.isArray(jid)) {
                ...
                const fullMsg = await generateWAMessage('status@broadcast', content, {
                    ...
                    ...options,
                    messageId: options.messageId || generateMessageIDV2(userJid) // <--- CORRECCIÓN AQUÍ
                });
                for (const id of jid) {
                    if (isJidGroup(id)) {
                        try {
                ...
                if ('album' in content) {
                    ...
                    for (const albumMedia of content.album) {
                        const albumMsg = await generateWAMessage(jid, albumMedia, {
                            ...
                            options: config.options,
                            ...options,
                            messageId: options.messageId || generateMessageIDV2(userJid) // <--- CORRECCIÓN AQUÍ TAMBIÉN
                        });
